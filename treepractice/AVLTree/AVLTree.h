...
private:
  ...
  //custom code: private helper method to allow traversal of nodes AND
  //provides automatic rebalancing.
  AVLNode* insert(AVLNode* n, const string& x);

  //custom code: private method to find the path to a node.
  //returns "" if such a node does not exist.
  string pathFromNode(AVLNode* n, const string& x) const;

  //custom code: finds a node with n as the root
  bool findValue(AVLNode* n, const string& x) const;

  //custom code: returns num of nodes with n as the root of the tree
  int numNodesRoot(AVLNode* n) const;
    // Any other methods you need...

  //custom code: calculates deepest node with a balance factor of -2 or + 2
  int calculateBL(AVLNode* n);
