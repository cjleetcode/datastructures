...
string getRot(int i, int j, int k);
...
string getRot(int i, int j, int k){
 switch (i) { // assumes grid[0][0] is in the upper-left
            case 0:
	      return "N (" + to_string(j) + ", " + to_string(k) + "):";
                 // north
            case 1:
	      return "NE(" + to_string(j) + ", " + to_string(k) + "):"; // north-east
            case 2:
	      return "E (" + to_string(j) + ", " + to_string(k) + "):";// east
            case 3:
	      return "SE(" + to_string(j) + ", " + to_string(k) + "):"; // south-east
            case 4:
	      return "S (" + to_string(j) + ", " + to_string(k) + "):"; // south
            case 5:
	      return "SW(" + to_string(j) + ", " + to_string(k) + "):"; // south-west
            case 6:
	      return "W (" + to_string(j) + ", " + to_string(k) + "):";// west
            case 7:
	      return "NW(" + to_string(j) + ", " + to_string(k) + "):";// north-west
 }
 return "";
}
 int main(int argc, char* argv[]){
  ...
  timer* t = new timer();
  bool gridExists = readInGrid(argv[2], rows, cols);
  ...
  int wordC = 0;
  int maxBound = max(rows, cols);
  if(max(rows, cols) == 3){
    maxBound = 4;
  }
  else{
  maxBound = min(maxBound, 23);
  }
  t -> start();
  for(int i = 0; i < rows; i++){
    for(int j = 0; j < cols; j++){
	for(int k = 0; k < 8; k++){
	  for(int l = 3; l < maxBound; l++){
	    string found = getWordInGrid(i, j, k, l, rows, cols);
	    if(h -> find(found)){
	    cout << getRot(k, i, j) << "    " << found << endl;
	    wordC++;
	    if(getWordInGrid(i, j, k, l + 1, rows, cols) == found)
	      break;
	    continue;
	  }
	}
